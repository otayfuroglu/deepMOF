...
mol_path ="/truba_scratch/otayfuroglu/deepMOF/HDNNP/prepare_data/nonEquGeometries/"
db_path = "/truba_scratch/otayfuroglu/deepMOF/HDNNP/prepare_data"
db_name = "nonEquGeometriesEnergyForcesWithORCAFromMD.db"
csv_name = "calculatedFilesFromMD.csv"

if not os.path.exists("%s/%s" %(db_path, csv_name)):
    df = pd.DataFrame(columns=["FileNames"]) 
    df.to_csv("%s/%s" %(db_path, csv_name))
...
def orca_calculator(label, n_cpu, initial_gbw=['','']):
    return ORCA(label=label,
                maxiter=2000,
                charge=0, mult=1,
                orcasimpleinput='SP PBE D4 DEF2-SVP DEF2/J RIJDX MINIPRINT NOPRINTMOS NOPOP NoKeepInts NOKEEPDENS ' + initial_gbw[0],
...
class CaculateData():
    ...
    def _caculate_data(self, file_name):
        ...
        try:
            ...
            self.i += 1
            self.new_db.add_system(mol, "%s"%file_base, energy=energy, forces=forces)
            os.system("rm %s*" %label)
        except:
