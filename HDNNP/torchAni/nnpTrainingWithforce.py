...
energy_shifter = torchani.utils.EnergyShifter(self_energies=self_energies)

path = os.getcwd()

dspath = os.path.join(path, './non_equ_geom_energy_coh_energy_forces_withORCA.h5')
...
Zn_network = torch.nn.Sequential(
    torch.nn.Linear(aev_dim, 128),
    torch.nn.CELU(0.1),
    torch.nn.Linear(128, 112),
    torch.nn.CELU(0.1),
...
nn = torchani.ANIModel([H_network, C_network, O_network, Zn_network])
...
AdamW = torch.optim.AdamW([
    ...
    # O networks
    {'params': [Zn_network[0].weight]},
    {'params': [Zn_network[2].weight], 'weight_decay': 0.00001},
    {'params': [Zn_network[4].weight], 'weight_decay': 0.000001},
    {'params': [Zn_network[6].weight]},
])
...
SGD = torch.optim.SGD([
    ...
    # O networks
    {'params': [Zn_network[0].bias]},
    {'params': [Zn_network[2].bias]},
    {'params': [Zn_network[4].bias]},
    {'params': [Zn_network[6].bias]},
], lr=1e-3)
...
force_coefficient = 0.5  # controls the importance of energy loss vs force loss
...
with h5py.File(dspath, "r") as f:
    #f.visititems(print_attrs)
    coordinate = f["non_equ_geom_energy_coh_energy_forces_withORCA/mof5_f1/coordinates"][0]
    species = f["non_equ_geom_energy_coh_energy_forces_withORCA/mof5_f1/species"][:]
    f.close()


#_, predicted_energy = model((species, coordinates))
